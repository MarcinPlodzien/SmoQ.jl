...
# Import bitwise gates for classical shadows rotations
using ..CPUQuantumChannelGates: apply_hadamard_psi!, apply_s_psi!, apply_sdagger_psi!

# Import sampling helper from measurements
using ..CPUQuantumStateMeasurements: sample_from_probabilities
...
function clifford_shadow_snapshot(ψ::Vector{ComplexF64}, N::Int)
    ...
    outcome_idx = sample_from_probabilities(probs) - 1  # 0-based
...
function clifford_shadow_snapshot(ρ::Matrix{ComplexF64}, N::Int)
    ...
    outcome_idx = sample_from_probabilities(probs) - 1
...
function classical_shadow_snapshot(ψ::Vector{ComplexF64}, N::Int)
    ...
    for k in 1:N
        if bases[k] == 1      # X basis: apply H
            apply_hadamard_psi!(ψ_work, k, N)
        elseif bases[k] == 2  # Y basis: apply S† then H
            apply_sdagger_psi!(ψ_work, k, N)
            apply_hadamard_psi!(ψ_work, k, N)
        end
    ...
    outcome_idx = sample_from_probabilities(probs) - 1  # 0-based for bit ops
...
function estimate_local_observables(snapshots::Vector{PauliSnapshot}, N::Int)
    ...
    for j in 1:N
        # Build Pauli strings
        pauli_X = zeros(Int, N); pauli_X[j] = 1
        pauli_Y = zeros(Int, N); pauli_Y[j] = 2
        pauli_Z = zeros(Int, N); pauli_Z[j] = 3
